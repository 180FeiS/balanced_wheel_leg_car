 ...
 * （用户）         ——               UART1：无线 VOFA 接收（wireless_module_uart_handler），与 CM7_0 工程一致
 ********************************************************************************************************************/
...
{
    /*
     * CM7_1 固件不得 pit_init(PIT_CH1)：CNT[1] 由 CM7_0 独占供 leg_control。
     * CM7_1 工程仍链接本符号（见 zf_driver_pit pit_isr_func）；本核不使能 CH1 时通常不会进入。
     * 勿在此清除 PIT_CH1 标志或操作 TCPWM 寄存器，以免与 CM7_0 所有者竞态。
     */
}
...
void uart1_isr(void)
{
    if (Cy_SCB_GetRxInterruptMask(get_scb_module(UART_1)) & CY_SCB_UART_RX_NOT_EMPTY) // 串口1接收中断
    {
        ...
        /* CM7_1：无线 VOFA / 逐飞无线模块数据入口（与 vofa.c 中 Wired_Mode=1 一致） */
