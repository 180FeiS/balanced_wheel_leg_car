 ...
 *   上升沿→当前朝向显示 0°，下降沿→当前朝向显示 180°；成功时翻转 LED1。
...
void motor_poll_switch2_speed_baseline(void); /* SWITCH2 边沿：上升沿显示 yaw=0°，下降沿显示 yaw=180°，成功翻转 LED1 */
...
void steer_request_target_yaw(float target_yaw_deg);        // 登记最新绝对航向；NAV_HEADING_MODE_GPS 时由 GPS_PointNav_Run 填目标（含发车后 COG 标定偏置），与惯导回放 N.Angle_Run 分离
