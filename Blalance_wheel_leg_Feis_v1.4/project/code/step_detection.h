 ...
 * pit0_ch0 每 ms 调用 step_visual_jump_post_jump_cooldown_on_cm7_1_1ms()，内部 5 分频等价每 5ms 减一档；时长仍为 N×5ms，不占 PIT_CH1）。
...
#endif
/* jump_active 从 1 变 0 后，再经 N 个「等效 5ms」节拍内禁止下沿触发（递减由 CM7_1 的 pit0_ch0×5 软件分频驱动）；冷却时长 = N×5ms；0=关闭。
...
/* CM7_1：pit0_ch0_isr（1ms）内每拍调用一次；内部做 5 次累计后递减落地冷却一档，等价原 PIT_CH1 真 5ms，避免与 CM7_0 leg_control 争用 TCPWM CNT[1] */
