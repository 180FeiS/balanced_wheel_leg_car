---
name: 里程纠偏计划
overview: 在不改动导航事件框架的前提下，为纯惯导回放增加基于左右轮速度与 IMU 角速度的打滑检测和里程纠偏，采用“瞬时保护 + 状态确认 + 回溯纠偏”的三层策略，优先修正转弯时单轮空转导致的 Run_index 超前问题。第一版不处理台阶开环跳跃腾空，也不启用 IMU 加速度积分，只预留扩展接口和注释。
todos:
  ...
  - id: integrate-corrected-step
    content: 在 navigation.c 增加瞬时保护、打滑状态确认和中心速度重建函数，并接入 Nag_GetMileageStep()
    status: completed
  - id: reset-and-debug
    content: 补齐初始化复位和调试输出规划，确保新增状态不会跨模式残留
    status: completed
  - id: rollback-window
    content: 设计短历史窗口与回溯扣账逻辑，避免短时腾空在确认前已把 Run_index 推超前
    status: completed
isProject: false
---
...
- 第一版策略不采用“检测到后才简单切换速度源”，而是前移为“先止损、再确认、最后补账”，优先压低短时腾空对 `Run_index` 的瞬时超前。
...
## 方案设计
### 1. 三层总体策略
- 第一层：瞬时保护。每个 1ms 周期在 `Mileage_Step` 真正累计前，先做一次一致性检查和步长限幅，避免可疑轮速当拍把 `Run_index` 提前推走。
- 第二层：状态确认。若左右轮速度与 `gyro_z` 的一致性连续若干 ms 异常，再正式判定为左轮打滑、右轮打滑或双侧不可信。
- 第三层：回溯纠偏。一旦确认打滑，对最近短窗口内“已经按原始步长多推进的里程”做补扣，尽量消除检测延迟造成的超前误差。
- 这样即使单轮腾空很短，也不会完全依赖“事后切换速度源”来止损。
...
- 在 `Nag` 结构体中新增“里程纠偏”相关状态，建议包含：
  ...
  - 进入/退出打滑的连续计数器
  - 最近一次可信速度
  - 最近若干 ms 的原始步长/纠偏步长历史窗口
  - 待补扣的里程误差累计量
- 状态命名与现有 `N.Event_*`、`N.Speed_Forward` 风格保持一致，避免引入独立全局变量。

### 3. 新增专门的里程纠偏函数
- 在 `navigation.c` 中新增 3~5 个静态函数，职责拆开：
  ...
  - 输出本周期纠偏后的里程步长
- 推荐接口形态：
  - `Nag_ProtectMileageStepInstant(...)`
  - `Nag_UpdateSlipDetection(...)`
  - `Nag_ApplySlipRollback(...)`
  - `Nag_GetCorrectedForwardSpeed(...)`
  - `Nag_GetCorrectedMileageStep(...)`
- `Nag_GetMileageStep()` 最终改为优先调用“纠偏后步长”，保留现有纯 `car_speed` 积分作为兜底路径；但即使在未正式进入打滑态前，瞬时保护层也应先对本拍步长做保守处理。
...
- 判定逻辑分为两级：
  - 快速级：单拍发现候选中心速度明显不一致时，立即限制当前步长，避免这一拍就把里程推超前。
  - 确认级：满足阈值持续若干 ms 才进入打滑；恢复也要求持续若干 ms，避免弯道边缘来回抖动。
- 打滑一旦确认，需要对短历史窗口中的原始步长与保守步长差值做累积，作为后续回溯扣账依据。
...
- `D:/Gitee/balanced_wheel_leg_car/Blalance_wheel_leg_Feis_v1.4/project/code/navigation.c`
  - 新增瞬时保护、打滑状态确认、回溯纠偏相关静态函数。
  - 在 `Nag_GetMileageStep()` 接入“先保护、再确认、必要时补扣”的纠偏速度计算。
...
- 先验证瞬时保护是否生效：即使还没正式进入打滑态，可疑轮速出现的当拍，`Mileage_Step` 也不能完整吃下原始 `car_speed` 的尖峰。
- 再验证状态确认与回溯纠偏：确认打滑后，最近窗口的超前里程应被补扣，`Run_index` 不应持续保持误超前。
- 实车重点观察三类场景：
  - 普通直行，不能比原方案更抖
  - 正常弯道，不能把正常差速误判为打滑
...
- 调试方法按“先静态、再低速、后动态”展开：
  ...
  - 正常弯道：重点看左右轮速度允许差速，但左右反推中心速度仍应接近；若打滑状态频繁跳变，优先调一致性阈值、瞬时保护阈值和去抖计数
  - 单轮短时空转：重点看原始 `car_speed` 是否突然抬高、某一侧反推中心速度是否明显偏离、瞬时保护后的步长是否已被限住、确认后是否对前几拍误差做了补扣
- 实车重点看三类量：
  - 原始量：左右轮速度、原始 `car_speed`、`gyro_z`
  - 判据量：左右反推中心速度、两侧一致性误差、打滑状态、瞬时保护后的步长
  - 结果量：最终纠偏中心速度、回溯补扣量、`Nag_GetMileageStep()` 输出或里程累计变化趋势
- 若通道数不够，优先级建议为：左右轮速度、`gyro_z`、左右反推中心速度、瞬时保护后的步长、最终纠偏速度；打滑状态和回溯补扣量可后续切第二组观察或替换其中一路。

## 后续扩展位
- 预留第二阶段接口：当后续确认可稳定取得“去重力后的前向加速度”时，再接入短时 IMU 积分兜底。
- 第二阶段只在“双轮都不可信且持续时间较短”时启用，超时仍回退到冻结/限幅推进策略；其接入位置也应放在“瞬时保护 + 状态确认 + 回溯纠偏”框架内，而不是替代当前三层策略。